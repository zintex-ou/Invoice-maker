...
final class ClassicTemplateViewModel: ObservableObject {
            ...
            for index in pages.0.indices {
                ...
                let view: AnyView = {
                    if index == 0 {
                        ...
                        return AnyView(
                            ClassicTemplateContinuationView(items: pages.0[index], startIndex: startIndices[currentPage], customColor: .constant(customColor))
                                .frame(width: pageSize.width,
                                       height: pageSize.height,
                                       alignment: .top)
                        )
